...
public class Database extends SQLiteOpenHelper {
    ...
    public static final String CYCLE_COL_5 = "price_per_hour";
    ...
    public static final String TRANSACTION_COL_7 = "price_per_hour";
    ...
    public boolean insertData_User(String first_name, String last_name, String email, String hall, String room,
            String username, String password, String mobile_number) {
    ...
    public boolean insertData_Admin(String first_name, String last_name, String email, String username,
            String password) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    if(model.isEmpty()||color.isEmpty()||username.isEmpty()||location.isEmpty())return false;

    ContentValues contentValues = new ContentValues();return false;

    contentValues.put(CYCLE_COL_1,reg_no);contentValues.put(CYCLE_COL_2,model);

    contentValues.put(CYCLE_COL_3,color);contentValues.put(CYCLE_COL_4,location);

    contentValues.put(CYCLE_COL_5,price_per_hour);contentValues.put(CYCLE_COL_6,username);
    SQLiteDatabase db = this
            .getWritableDatabase();contentValues.put(CYCLE_COL_7,0);contentValues.put(CYCLE_COL_8,condition);
    int t_id = rand.nextInt(1000);
    long result = db.insert(TABLE_CYCLE, null, contentValues);

    if(result==-1)return false;
    long result = db.insert(TABLE_TRANSACTION, null, contentValues);else return true;
    }

    public boolean updateData_User(String username, String first_name, String last_name, String email, String hall,

    public boolean insertData_Transaction(String username, String owner, String reg_no, String start_date, String end_date, int price_per_hour){
        SQLiteDatabase db = this.getWritableDatabase();
        ...
        contentValues.put(TRANSACTION_COL_7, price_per_hour);
    ...
    public boolean updateData_Cycle(String reg_no, String model, String color, String location, int price_per_hour,String condition) {
        SQLiteDatabase db = this.getWritableDatabase();
    ...
    }contentValues.put(CYCLE_COL_2,model);contentValues.put(CYCLE_COL_3,color);

    SQLiteDatabase db = this
            .getWritableDatabase();contentValues.put(CYCLE_COL_4,location);contentValues.put(CYCLE_COL_5,price_per_hour);

    contentValues.put(CYCLE_COL_8,condition);db.update(TABLE_CYCLE,contentValues,"reg_no=?",new String[]{reg_no});return true;}

    public void updateData_Cycle_rating(String reg_no) {
        SQLiteDatabase db = this.getWritableDatabase();

        db.execSQL("UPDATE " + TABLE_CYCLE + " SET rating=(SELECT avg(cycle_rating) FROM " + TABLE_TRANSACTION
                + " WHERE reg_no=" + Integer.parseInt(reg_no) + ") WHERE reg_no=" + Integer.parseInt(reg_no) + ";");
    }
    ...
    public boolean insertData_Location(String name) {
        ...
        ContentValues contentValues = new ContentValues();
                new String[] { email, q_Date, subject });
    ...
    public int deleteData_Contact(String email, String subject, String q_Date) {
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(TABLE_CONTACT, "email_id=? AND query_date=? AND subject=?",
                new String[] { email, q_Date, subject });
    }
    ...
    public boolean login_User(String username, String password) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT * FROM " + TABLE_USER + " WHERE username=? AND passw=?",
                new String[] { username, password });
    ...
    public boolean login_Admin(String username, String password) {
        Cursor result = db.rawQuery("SELECT * FROM " + TABLE_ADMIN + " WHERE username=? AND passw=?",
                new String[] { username, password });
        if (result.getCount() == 1)
            return true;
        else
    ...
    public Cursor getAllData_User_cycle() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT username, count(reg_no) as c from " + TABLE_USER
        return result;
    }
    ...
    public Cursor getRegNo_Cycle_username(String username) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT reg_no FROM " + TABLE_CYCLE + " WHERE username=?",
                new String[] { username });
        return result;
    }
    ...
    public Cursor getData_User_mobile_number(String mobile_number) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT * FROM " + TABLE_USER + " WHERE mobile_number=?",
                new String[] { mobile_number });
        return result;
    }
    ...
    public Cursor getData_Cycle_GetRide(String username, String date) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db
                .rawQuery("SELECT * FROM " + TABLE_CYCLE + " WHERE username!=? AND reg_no NOT IN (SELECT reg_no FROM "
                        + TABLE_TRANSACTION + " WHERE end_date>=?)", new String[] { username, date });
        return result;
    }

    public Cursor getData_Transaction_delete(String reg_no, String date) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT * FROM " + TABLE_TRANSACTION + " WHERE reg_no=? AND end_date>=?",
                new String[] { reg_no, date });
        return result;
    }
    ...
    public Cursor getData_Transaction_user_delete(String username, String date) {
        ...
        Cursor result = db.rawQuery(
                "SELECT * FROM " + TABLE_TRANSACTION + " WHERE (username=? OR owner=? ) AND end_date>=?",
                new String[] { username, username, date });
        return result;
    }

    public Cursor getData_Transaction_ride(String username) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT * FROM " + TABLE_TRANSACTION + " WHERE username=? ORDER BY end_date DESC",
                new String[] { username });
        return result;
    }
    ...
    public Cursor getData_Transaction_rent(String owner) {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor result = db.rawQuery("SELECT * FROM " + TABLE_TRANSACTION + " WHERE owner=? ORDER BY end_date DESC",
                new String[] { owner });
        return result;
    }
}